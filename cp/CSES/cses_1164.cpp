...
void soln(ll t){
    ...
    I n;
    V(ll) arr(n);
    V(P2(ll,P(ll))) timeline;
    F(i,n){
        I a>>b;
        timeline.push_back({a,{1,i}});
        timeline.push_back({b,{2,i}});
    }
    Srt(timeline);

    V(ll) roomallocated(n,0),emptyroom;
    ll total_rooms=0;
    Fa(xx,timeline){
        if(roomallocated[xx.second.second]){
            emptyroom.push_back(roomallocated[xx.second.second]);
        }else{
            if(emptyroom.empty()){
                total_rooms++;
                roomallocated[xx.second.second]=total_rooms;
            }else{
                roomallocated[xx.second.second]=emptyroom.back();
                emptyroom.pop_back();
            }
        }
    }
