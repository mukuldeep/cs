...
ll is_valid_parent(ll par[],ll lca,ll curr){
    while(curr!=lca){
        curr=par[curr];
        if(curr==-1)
            return 0;
...
void sum_of_child(ll a[],vector<ll> ch[],ll &sum_b,ll chl){//including self
    sum_b+=a[chl];
...
void sub_child(ll a[],vector<ll> ch[],ll &sum_b,ll chl){//including self
    sum_b-=a[chl];
    for(auto chld:ch[chl]){
        sub_child(a,ch,sum_b,chld);
    }
}
...
//    if(!is_valid_parent(par,x,i))return 0;
//
//    ll sum_b=0;
//    sum_of_child(a,ch,sum_b,x);
//    if(x!=i) {
//        sub_child(a, ch, sum_b, i);
//        sum_b+=a[i];
//    }
//    return sum_b;
