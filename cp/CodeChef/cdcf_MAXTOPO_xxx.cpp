...
ll dfss2(vector<vector<ll>> &adj,vector<ll> &vis,ll node_n,vector<ll> &nos,vector<ll> &invpart,vector<ll> &allchildn,vector<ll> &nos_ch){
    vis[node_n]=1;
    ll chcnt=0,an=1,l_invpart=1,all_chcnt=0,nos_ch_all=1;

    for(auto o: adj[node_n]){
        if(!vis[o]){
            ll l_an=dfss2(adj,vis,o,nos,invpart,allchildn,nos_ch);
            an*=l_an;an%=t9p7;
            nos_ch_all*=nos_ch[o];nos_ch_all%=t9p7;
            chcnt++;
            l_invpart*=inv_fac[allchildn[o]+1];l_invpart%=t9p7;
            all_chcnt+=allchildn[o];
        }
    ...
    nos_ch_all*=an;nos_ch_all%=t9p7;
    nos_ch[node_n]=nos_ch_all;
    allchildn[node_n]=all_chcnt+chcnt;
    invpart[node_n]=l_invpart;
    ...
    this_nos*=fac[allchildn[node_n]];this_nos%=t9p7;
    this_nos*=l_invpart;this_nos%=t9p7;
    this_nos*=nos_ch_all;this_nos%=t9p7;
    nos[node_n]=this_nos;
    return this_nos;
}
