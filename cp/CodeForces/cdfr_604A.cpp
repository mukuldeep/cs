...
void soln(ll t){
    ll n,k,a,b,c,u,d,l,r,cnt=0,mn=LONG_LONG_MAX,mx=LONG_LONG_MIN;
    ld ans=0.0;
    V(ld) m(5),w(5),h(2),pr={500,1000,1500,2000,2500};
    F(i,5)I m[i];
    F(i,5)I w[i];
    F(i,2)I h[i];

    F(i,5)ans+=max(0.3*(ld)pr[i],(1-(ld)m[i]/250.0)*pr[i]-50.0*w[i]);
    ans+=100*h[0];
    ans-=50*h[1];
    O round(ans)<<El
}
