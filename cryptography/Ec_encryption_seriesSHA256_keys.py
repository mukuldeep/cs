...
class crypto:
    ...
    # decryption of message to be send
    ...
    ###################################################################################
    ################   EC Encryption using series of SHA256 keys      #################
    ###################################################################################
    #encryption(series of SHA256) of message to be send
    #Currently this is only xor(NOT SECURE), it is to be changed to ECadd & ECsub
    def encrypt_ser_message(self,msg,r,prv_k,pub_k):
        (px,py)=self.ECmultp(pub_k[0],pub_k[1],prv_k)
        (rx,ry)=self.ECmultp(px,py,r)
        enc_msg=0
        l=0
        ...
        for ch in msg:
            ...
            if(cnt==30):
                ...
                l=l^rx
                # enc_msg <<= 256
                enc_msg|=(l)<<(256*blockno)
                blockno+=1
                l=0
                #manipulate rx
                rx=int("0x"+str(hashlib.sha256(str(rx).encode()).hexdigest()),16)
                # print("rx",rx)
        if(l):
            l = l ^ rx
            # enc_msg <<= 256
    ...
    ###################################################################################
    ################   EC Decryption using series of SHA256 keys      #################
    ###################################################################################
    # decryption(series of SHA256) of message to be send
    # Currently this is only xor(NOT SECURE), it is to be changed to ECadd & ECsub Concept
    def decrypt_ser_message(self,enc_msg,r,prv_k,pub_k):
        (px, py) = self.ECmultp(pub_k[0], pub_k[1], prv_k)
        (rx, ry) = self.ECmultp(px, py, r)
        msg=""
        while(enc_msg):
